...
public class Lambda_01_Test {
	...
	public void test_filter_by_age() throws Exception {
		...
		PersonPredicate test = p -> p.getAge() >= 18;

		List<Person> result = filter(personList, test);
	...
	public void test_filter_by_firstname() throws Exception {
		...
		// "first_10"
		PersonPredicate test = p -> p.getFirstname().equals("first_10");
		List<Person> result = filter(personList, test);
	...
	public void test_filter_by_password() throws Exception {
		...
		PersonPredicate test = p -> p.getAge() > 49 && DigestUtils.sha512Hex(p.getPassword()).equals(passwordSha512Hex);
		List<Person> result = filter(personList, test);
